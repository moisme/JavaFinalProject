...
@NamedQuery(name = MedicalCertificate.ALL_CERTIFICATES_QUERY_NAME, query = "SELECT mc FROM MedicalCertificate mc left join fetch mc.medicalTraining")
@NamedQuery(name = MedicalCertificate.ID_CARD_QUERY_NAME, query = "SELECT mc FROM MedicalCertificate mc left join fetch mc.medicalTraining where mc.id = :param1")
@NamedQuery(name = "MedicalCertificate.findByPhysicianId", query = "SELECT mc FROM MedicalCertificate mc WHERE mc.owner.id = :physicianId")
