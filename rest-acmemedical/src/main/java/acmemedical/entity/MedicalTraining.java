...
@NamedQuery(name = MedicalTraining.FIND_ALL, query = "SELECT mt FROM MedicalTraining mt left join fetch mt.school left join fetch mt.school")
@NamedQuery( name = MedicalTraining.FIND_BY_ID, query = "SELECT mt FROM MedicalTraining mt left join fetch mt.school left join fetch mt.school where mt.id = :param1")
...
public class MedicalTraining extends PojoBase implements Serializable {
	...
	@OneToOne(mappedBy = "medicialTraining")
	@JsonIgnore
